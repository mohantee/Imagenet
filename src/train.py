...
def train_transforms(augment: bool = True):
...
# Mixup augmentation (kept but alpha reduced)
...
def train(model, train_loader, criterion, optimizer, scheduler, device, epoch,
    ...
    for batch_idx, (inputs, targets) in enumerate(pbar):
        # Move to device and channels-last
        ...
        if ema_model is not None:
            with torch.no_grad():
                for ema_p, model_p in zip(ema_model.parameters(), model.parameters()):
                    ema_p.mul_(ema_decay).add_(model_p.data, alpha=1 - ema_decay)

        # Accuracy approx for mixup
